...
function _updateGroup(board, groupId, prop, toUpdate) {
  let group = board.groups.find((group) => groupId === group.id)
  if (prop !== 'tasks') {
    let activity = addActivity(prop, group.title, group[prop], toUpdate)
    board.activities.unshift(activity)
  }
  group[prop] = toUpdate
  return board
...
function updateBoard(currBoard, groupId, taskId, prop, toUpdate) {
  ...
  if (taskId) return _updateTask(board, groupId, taskId, prop, toUpdate)
  else if (groupId) return _updateGroup(board, groupId, prop, toUpdate)
