<template>
  <section class="activity-log grid">
    <article v-for="activity in activities" class="activity grid">
      <div>
        <i></i>
        <div v-tooltip="dateFormatted(activity.createdAt).tooltip"> {{ dateFormatted(activity.createdAt).date }} </div>
      </div>
...
export default {
  ...
  computed: {
    dateFormatted() {
      return (createdAt) => {
        let time = new Intl.DateTimeFormat('en-He', { timeStyle: 'short', hour12: true }).format(createdAt)
        let date = new Intl.DateTimeFormat('en-He', { month: 'short', day: 'numeric' }).format(createdAt)
